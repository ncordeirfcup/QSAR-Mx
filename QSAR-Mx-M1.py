...
def compout(df,n,c1,c2,sp):
    ...
    if len(nl4)==0:
        messagebox.showinfo('Error','Select other parameter values')
    else:
...
def datasel(callback):
    ...
    seed=secondEntryTabOne_y.get()
    interval=secondEntryTabOne.get()
    tsp.to_csv(str(a_)+str('_')+str(method)+'_division.csv',index=False)
    savename1= str(a_)+str(method)+'_s_'+str(seed)+'_i_'+str(interval)+'_tr.csv'
    #savename1='tr.csv'
    trdp.to_csv(savename1,index=False)
    #savename2='ts.csv'
    savename2= str(a_)+str(method)+'_s_'+str(seed)+'_i_'+str(interval)+'_ts.csv'
...
def writefilex(callback):
    ...
    df.to_csv(str(c_)+"_trpr.csv",index=False)
    ...
    filer.write('Maximum intercorrelation between descriptors: '+str(mx)+"\n")
    ...
    if ytr.columns[0] in file2.columns:
       ...
       dftsm2=dfts[dfts['Outlier_info(standardization_approach)']=='Outlier']
       dfts.to_csv(str(c_)+"_tspr.csv",index=False)
       ...
       pyplot.savefig(rocn, dpi=300, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, \
        ...
        dfts.to_csv(str(c_)+"_scpr.csv",index=False)
...
fifthLabelTabThreer6c2.place(x=80,y=160)
fifthBoxTabThreer6c2= Spinbox(tab2, from_=0, to=100, width=5)
fifthBoxTabThreer6c2.place(x=200,y=160)

fifthLabelTabThreer8c2 = Label(tab2, text= '% MAE(LOO) reduction',font=("Helvetica", 12))
fifthLabelTabThreer8c2.place(x=250,y=160)
fifthLabelTabThreer9c2=Entry(tab2)
fifthLabelTabThreer9c2.place(x=420,y=160)

var3= IntVar()
N1 = Checkbutton(tab2, text = "Y-randomization",  variable=var3, \
                 font=("Helvetica", 12), command=enable3)
N1.place(x=480, y=180)

N1B1 = Label(tab2, text= 'Number of Runs',font=("Helvetica", 12))
N1B1.place(x=490,y=200)
N1B1_x=Entry(tab2,state=DISABLED)
N1B1_x.place(x=490,y=230)

fifthLabelTabThreer7c2 = Label(tab2, text= 'Cross validation',font=("Helvetica", 12))
