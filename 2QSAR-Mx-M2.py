...
def seliter():
    ...
    for i in range(1,int(secondEntryTabOne.get())):
            for j in range(1,int(secondEntryTabOne_y.get())):
                file1,file2=datasel(i,j,filex1,directory)         ######Entry
                ...
                if file1.shape[0]>file2.shape[0]:
                   if ratio>20:
                      ...
                      process(file1,file2,filer,cfile,i,j)
                      filer.close()
                   else:
                      continue
                else:
...
def process(file1,file2,filer,cfile,i,j):
    #Xtr1=file1.iloc[:,3:]
    #Xtr2=file1.iloc[:,1:2]
    #Xtr=pd.concat([Xtr1,Xtr2],axis=1)
    #Xtr=Xtr.reset_index().drop('index',axis=1)
    ...
    #df.to_csv(str(a_)+"_sfslda_trpr.csv",index=False)
    ...
    cfile.write('Maxmimum intercorrelation between descriptors: '+str(mx)+"\n")
    ...
    if ytr.columns[0] in file2.columns:
       ...
       #dfts.to_csv(str(a_)+"_sfslda_tspr.csv",index=False)
...
b2=Button(tab1, text='Generate model', command=seliter,bg="orange",font=("Helvetica", 10),anchor=W, justify=LEFT)
